...
ytmlink=""
google=false
while getopts ":ht:l:o:y:g-:" arg; do
    case $arg in
        t)
            timer=${OPTARG}
            ;;
...
do
    ...
    DISPLAY=:2.1 ${browser} --no-sandbox --user-data-dir="/root/chromeTE" --user-agent="Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/49.0.2623.87 Safari/537.36" --disable-popup-blocking --incognito ${ytmlink} ${otolink} ${links} & disown
    browserPID=$!
    sleep ${timer}
